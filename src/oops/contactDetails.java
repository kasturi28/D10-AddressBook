...
public class contactDetails {
	ArrayList<personalInformation> persons;
	Scanner scanner = new Scanner(System.in);

	public contactDetails() {
		persons = new ArrayList<personalInformation>();
	}
	...
	public void addPerson() {
		...
		personalInformation p = new personalInformation(name, lName, address, city, state, zip, number, email);
		persons.add(p);
	...
	public void show() {
		...
		if (persons.size() == 0) {
			...
			for (int i = 0; i < persons.size(); i++) {
				personalInformation p = (personalInformation) persons.get(i);
	...
	public void delete() {
		if (persons.size() == 0) {
			...
			String name = scanner.nextLine();
			for (int i = 0; i < persons.size(); i++) {
				personalInformation p = persons.get(i);
